<?php
// Temporary upload handler without reCAPTCHA for testing
...
// Log for debugging
error_log('Upload temp - POST data: ' . print_r($_POST, true));
error_log('Upload temp - FILES data: ' . print_r($_FILES, true));
...
try {
    ...
    $emailContent = "Uus metsakava hindamise päring metsamaahind.ee-st (TEMP - no reCAPTCHA):\n\n";
    ...
    $subject = 'TEST - Metsakava hindamine - metsamaahind.ee';
    ...
    if (mail($to, $subject, $emailContent, $headers)) {
        http_response_code(200);
        echo json_encode(['message' => 'TEST: Metsakava ja vormandmed on edukalt saadetud!']);
    } else {
    ...
    error_log('Upload temp form error: ' . $e->getMessage());
    http_response_code(500);
    echo json_encode(['error' => 'Vormi saatmisel tekkis viga: ' . $e->getMessage()]);
}
?>